#!/usr/bin/env bash
# DFlash speculator smoke launcher. max_model_len stays at 400000: the tiered
...
# method == "mtp" (tiered_moe_physical.py), so the ~6.3 GB bf16 DFlash draft is
...
draft="${models_dir}/GLM-5.2-FP8-DFlash"
...
draft_backend="${DFLASH_DRAFT_BACKEND:-FLASH_ATTN}"
...
# under DFlash, so it costs nothing but the fingerprint has to match.
...
# planner does not budget the DFlash draft weights. Raising the HBM reserve
