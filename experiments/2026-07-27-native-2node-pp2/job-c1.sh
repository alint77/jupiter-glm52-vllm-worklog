...
#SBATCH --job-name=native-pp2-c1
#SBATCH --output=agent_space/experiments/2026-07-27-native-2node-pp2/slurm-c1-%j.out
#SBATCH --error=agent_space/experiments/2026-07-27-native-2node-pp2/slurm-c1-%j.err

# Native GLM-5.2 W4A16, 2 nodes, TP4 x PP2, batch one (c1), no offload.
# sbatch this file; it sruns run-server.sh once per node.
...
echo "[$(date)] native-pp2 c1 job ${SLURM_JOB_ID} nodes=${SLURM_JOB_NODELIST} master=${MASTER_ADDR}:${MASTER_PORT}"
